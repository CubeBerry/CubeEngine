...
{
	glm::vec2 windowSize = { static_cast<float>(Engine::GetCameraManager().GetCamera()->GetViewSize().x),
		static_cast<float>(Engine::GetCameraManager().GetCamera()->GetViewSize().y) };
	windowSize = (windowSize / Engine::GetCameraManager().GetCamera()->GetZoom());
	glm::vec2 cameraCenter = Engine::GetCameraManager().GetCamera()->GetCenter();
...
void BackgroundManager::AddVerticalParallexBackground(std::string spriteName_, std::string groupName, glm::vec2 position_, glm::vec2 size_, float angle_, float speedY_, float depth_, bool isAnimated)
{
	...
	glm::vec2 windowSize = { static_cast<float>(Engine::GetCameraManager().GetCamera()->GetViewSize().x) , static_cast<float>(Engine::GetCameraManager().GetCamera()->GetViewSize().y) };
	...
	if (isEditorMod == false)
	{
		for (auto& group : verticalParallaxBackgroundList)
		{
			for (auto& parallax : group.second)
			{
				...
				if (parallax.position.x <= -(Engine::GetCameraManager().GetCamera()->GetViewSize().x / 2.f)
					+ Engine::GetCameraManager().GetCamera()->GetCenter().x - parallax.size.x / 2.f)
				{
					parallax.position.x = Engine::GetCameraManager().GetCamera()->GetViewSize().x / 2.f + parallax.size.x / 2.f + Engine::GetCameraManager().GetCamera()->GetCenter().x;
				}
			}
		}
