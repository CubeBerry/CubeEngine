...
{
	m_workGraphsStateObject = std::make_unique<DXWorkGraphsStateObject>(m_device, "../Engine/shaders/cso/WorkGraphs.cso", L"WorkGraphsTutorial");

	std::vector<uint32_t> initData(1024, 0);
	// 1 SRV + 1 UAV
	auto [startCpuHandle, startIndex] = AllocateSrvHandles(2);
...
{
	if (!m_workGraphsEnabled) return;

	D3D12_SET_PROGRAM_DESC programDesc = m_workGraphsStateObject->GetProgramDesc();
	m_commandList->SetProgram(&programDesc);

	m_commandList->SetComputeRootSignature(m_workGraphsStateObject->GetGlobalRootSignature());
	...
	m_commandList->CopyResource(m_workGraphsReadBackBuffer.Get(), m_workGraphsOutputBuffer->GetStructuredBuffer());
