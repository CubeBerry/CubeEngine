...
{	
	...
	{
		animations[currAnim]->Update(dt);
		Engine::Instance().GetVKRenderManager()->GetVertexVector()->at(matrixId).frameSize = glm::vec4(GetFrameSize() / textureSize, 0.f, 0.f);
		Engine::Instance().GetVKRenderManager()->GetVertexVector()->at(matrixId).texelPos = glm::vec4(GetFrameTexel(animations[currAnim]->GetDisplayFrame()) / textureSize, 0.f, 0.f);
	}
}
...
void Sprite::UpdateModel(glm::vec3 pos_, glm::vec3 size_, float angle, int index)
{
	...
	Engine::Instance().GetVKRenderManager()->GetVertexVector()->at(index).model = modelMatrix;
}

void Sprite::UpdateView(int index)
{
	Engine::Instance().GetVKRenderManager()->GetVertexVector()->at(index).view = Engine::Engine().GetCameraManager()->GetViewMatrix();
}

void Sprite::UpdateProjection(int index)
{
	Engine::Instance().GetVKRenderManager()->GetVertexVector()->at(index).projection = Engine::Engine().GetCameraManager()->GetProjectionMatrix();
}
